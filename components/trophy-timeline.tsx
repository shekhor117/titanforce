      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12 sm:mb-16">
          <div>
            ...
            <p className={`text-foreground/60 text-lg ${isBn ? "font-[var(--font-bengali)]" : ""}`}>
              ...
              {trophies.map((trophy) => (
                <CarouselItem key={trophy.id} className="pl-2 md:pl-4 basis-full md:basis-1/2 lg:basis-1/3">
                  <div className="h-full">
                    <div className="relative p-8 rounded-2xl bg-card/50 backdrop-blur-sm border border-border/50 hover:border-primary/50 transition-all group/card hover:shadow-lg hover:shadow-primary/20 h-full flex flex-col hover-lift">
                      {/* Year Badge */}
                      <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full bg-gradient-to-r from-primary/20 to-accent/20 border border-primary/30 w-fit">
                        <span className="text-sm font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-accent">{trophy.year}</span>
                      </div>

                      {/* Trophy Icon */}
                      <div className="flex justify-center items-center w-20 h-20 mb-6 rounded-xl bg-gradient-to-br from-primary/30 to-accent/20 group-hover/card:from-primary/40 group-hover/card:to-accent/30 transition-colors duration-300">
                        <Trophy className="w-10 h-10 text-primary drop-shadow-lg" />
                      </div>

                      {/* Trophy Name */}
                      <h3 className="font-bold text-xl text-foreground mb-3 group-hover/card:text-primary transition-colors line-clamp-2">
                        {trophy.name}
                      </h3>
                      ...
                      {/* Description */}
                      <p className="text-sm text-foreground/70 mb-4 line-clamp-2 flex-grow leading-relaxed">
                        {trophy.description}
                      </p>

                      {/* Runners Up */}
                      {trophy.runners_up && (
                        <div className="text-xs text-muted-foreground mb-4 pb-4 border-t border-border/30">
                          <span className="font-semibold text-foreground/60">{isBn ? 'রানার আপ' : 'Runners up'}: </span>
                          <p className="mt-1 text-foreground/50">{trophy.runners_up}</p>
                        </div>
                      )}
